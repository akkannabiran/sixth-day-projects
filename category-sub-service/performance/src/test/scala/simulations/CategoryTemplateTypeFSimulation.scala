...
class CategoryTemplateTypeFSimulation extends Simulation {

  private val httpConf = http.baseURL(Config.categoryTemplateServiceBaseUrl)
  private val usersPerSec = users.toDouble

  def randomCategoryIdF = {
    val rows = CSVDictReader(Config.getResourcePath("categoryIdsF.csv")).toList
    shuffle(rows).head.get("categoryIds").get
  }

  private val categoryActionsScenario = scenario("Category Template - F")
    .exec { session =>
      session.set("categoryIdPayloadF", randomCategoryIdF)
    }
    .exec(GetCategoryTemplateDetail.apply(randomCategoryIdF))
