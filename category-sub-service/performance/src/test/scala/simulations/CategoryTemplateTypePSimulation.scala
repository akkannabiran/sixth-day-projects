...
class CategoryTemplateTypePSimulation extends Simulation {

  private val httpConf = http.baseURL(Config.categoryTemplateServiceBaseUrl)
  private val usersPerSec = users.toDouble

  def randomCategoryIdP = {
    val rows = CSVDictReader(Config.getResourcePath("categoryIdsP.csv")).toList
    shuffle(rows).head.get("categoryIds").get
  }

  private val categoryActionsScenario = scenario("Category Template - P")
    .exec { session =>
      session.set("categoryIdPayloadP", randomCategoryIdP)
    }
    .exec(GetCategoryTemplateDetail.apply(randomCategoryIdP))
