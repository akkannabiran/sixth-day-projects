...
public class LogCapture {
    ...
    private static PatternLayout getBasicPatternLayout() {
        PatternMatch[] patterns = {PatternMatch.newBuilder().setKey("ContentSyncLogMarker")
                .setPattern(
                        "%d{dd MMM yyyy HH:mm:ss,SSS} %-5p [%c] (%t) %notEmpty{MessageId=\"%X{CS.messageId}\", MessageType=\"%X{CS.messageType}\", ContextId=\"%X{CS.contextId}\", OriginTimestamp=\"%X{CS.originTimestamp}\", sixthdaySRC=\"%X{CS.sixthdaySRC}\", sixthdayDEST=\"%X{CS.sixthdayDEST}\", sixthdayRESOURCE=\"%X{CS.sixthdayRESOURCE}\",} %msg%n%throwable").build(),
